...
VCF_IN="$DATA/test_16_hsv1_XX.bcf.vcf"
BAM="$DATA/test_16_hsv1_XX.bam"
BAI="$DATA/test_16_hsv1_XX.bam.bai"
...
[[ "$n_lines" -eq 4 ]] || { echo "FAIL: expected 4 output variants, got $n_lines"; cat "$OUT"; exit 1; }
...
[[ "${CT[ACG]:-}" == "10" ]] || { echo "FAIL: expected ACG HAP_CT=10"; cat "$OUT"; exit 1; }

echo "OK test_16_multiallelic_same_pos"
